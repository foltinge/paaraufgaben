...
%s\\""" % (ueberschrift,namen[k],auftrag),file=f)
        for i,j in paare:
            if i==k:
                ...
                ri=aufgri[(i,j)]
                if ri:
                    print(r"""\begin{center}\underline{\phantom{%s}}
%s%s\end{center}"""%(aa[0],trenner,aa[1]),file=f)
                else:
                    print(r"""\begin{center}%s%s
\underline{\phantom{%s}}\end{center}"""%(aa[0],trenner,aa[1]),file=f)
            if j==k:
...
%s%s\end{center}"""%(aa[0],trenner,aa[1]),file=f)
                    ...
                    print(r"""\begin{center}%s%s\underline{\phantom{%s}}
